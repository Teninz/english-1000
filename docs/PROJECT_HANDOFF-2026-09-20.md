...
- Основной раздел = Oxford 3000 (3810 единиц «слово + часть речи», `tools/oxford-3000.json`) по уровням A1–B2
  и блокам по 20–25; **A1 готов полностью** (`oxford-a1.js`, 46 блоков), A2–B2 — пустые заготовки «готовится».
...
- Тесты: 58 (`npm test`), кэш сайта `shadowfox-v21`. Версия APK не поднималась — публиковать только после A2–B2.

Следующие шаги: контент A2 → B1 → B2 (по `OXFORD_PROGRAM.md` §6), затем тематические маршруты без добора по индексам
(§7), затем пункты 2–6 и 8 плана (входная проверка, достижения за устойчивые знания, лис с домиком, главная кнопка,
