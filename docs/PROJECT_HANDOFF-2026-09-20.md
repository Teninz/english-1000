...
## 9. Программа Oxford 3000 (20.09.2026, вечер) — не закоммичено на момент записи

Пункты 1 и 7 плана продолжения начаты; описание модели, переноса прогресса и процедуры добавления уровней —
`docs/OXFORD_PROGRAM.md` (читать перед продолжением). Кратко:

- Основной раздел = Oxford 3000 (3810 единиц «слово + часть речи», `tools/oxford-3000.json`) по уровням A1–B2
  и блокам по 20–30; **все четыре уровня готовы (21.09)**: `oxford-a1.js` … `oxford-b2.js`, 144 блока; контент A2–B2
  написан за один проход и нуждается в вычитке.
- 482 слова прежнего курса вне Oxford 3000 — `words-extra.js` («Дополнительный словарь», прежние 20 тем).
- Ключ прогресса — `слово|часть речи`; перенос v2→v3 в `LearningCore.migrate`, покрыт тестом.
- Новые задания «Устойчивые сочетания» и «Близкие значения», запасные примеры для сложных слов (`practice`, `confusables` в файле уровня).
- Инструменты: `tools/import-oxford-3000.py`, `program-scaffold.js`, `program-check.js`, `build-extra-words.js`.
- Тесты: 58 (`npm test`), кэш сайта `shadowfox-v21`. Версия APK не поднималась.

Следующие шаги: тематические маршруты без добора по индексам (`OXFORD_PROGRAM.md` §7), вычитка A2–B2,
затем пункты 2–6 и 8 плана (входная проверка, достижения за устойчивые знания, лис с домиком, главная кнопка,
нагрузка по результатам, бережное возвращение).
